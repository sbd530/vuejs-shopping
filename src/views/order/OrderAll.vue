<template>
  <div class="pom">
    <b-container class="bv-example-row">
      ...
      <b-row class="box">
        <b-row>
          <b-col>Order-All</b-col>

          <b-col align="right">
            <b-form-select v-model="max" class="mb-3">
          ...
          <vs-table v-model="selected">
            ...
            <template #tbody>
              <vs-tr
                :key="i"
                v-model="orders"
                v-for="(order, i) in $vs.getPage($vs.getSearch(orders, search),page,max)"
                :data="order"
                class="table"
              >
                <vs-td>
            ...
            <template #footer>
              <vs-pagination
                v-model="page"
                :length="$vs.getLength($vs.getSearch(orders, search), max)"
              />
            </template> </vs-table
        ></b-col>
      </b-row>
...
export default {
  ...
  methods: {
    ...
    cancelOrder: function (index) {
      if (this.orders[index].deliveryStatus == "배송완료") {
        alert("이미 배송이 완료되었습니다.");
        return;
      }
      http
